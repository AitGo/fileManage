...
public class HorizontalSelectedView extends View {
    ...
    private void initAttrs(AttributeSet attrs) {
        @SuppressLint("RestrictedApi") TintTypedArray tta = TintTypedArray.obtainStyledAttributes(this.getContext(), attrs, R.styleable.HorizontalselectedView);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.index >= 0 && this.index <= this.strings.size() - 1) {
            String s = (String) this.strings.get(this.index);
            ...
            canvas.drawText((String) this.strings.get(this.index), (float) (this.getWidth() / 2 - centerTextWidth / 2) + this.anOffset, (float) (this.getHeight() / 2 + this.centerTextHeight / 2), this.selectedPaint);

            for (int i = 0; i < this.strings.size(); ++i) {
                if (this.index > 0 && this.index < this.strings.size() - 1) {
                    this.textPaint.getTextBounds((String) this.strings.get(this.index - 1), 0, ((String) this.strings.get(this.index - 1)).length(), this.rect);
                ...
                if (i == 0) {
                    this.textPaint.getTextBounds((String) this.strings.get(0), 0, ((String) this.strings.get(0)).length(), this.rect);
                    this.textHeight = this.rect.height();
                }

                if (i != this.index) {
                    canvas.drawText((String) this.strings.get(i), (float) ((i - this.index) * this.anInt + this.getWidth() / 2 - this.textWidth / 2) + this.anOffset, (float) (this.getHeight() / 2 + this.textHeight / 2), this.textPaint);
                }
            }
        }
    ...
    public void setOffset(int offset) {
        if (this.index > 0 && offset > 0 && offset < strings.size()) {
            this.index = offset;
            this.invalidate();
        }
    ...
    public String getSelectedString() {
        return this.strings.size() != 0 ? (String) this.strings.get(this.index) : null;
    }
